...
## Program :
Developed By : MADHAN BABU P Register no. :212222230075
...
// grid 2D block 2D global void sumMatrixGPU(float *MatA, float *MatB, float *MatC, int nx, int ny) { unsigned int ix = threadIdx.x + blockIdx.x * blockDim.x; unsigned int iy = threadIdx.y + blockIdx.y * blockDim.y; unsigned int idx = iy * nx + ix;
```
if (ix < nx && iy < ny)
{
...
int main(int argc, char **argv) { printf("%s Starting ", argv[0]);
```
// set up device
int dev = 0;
